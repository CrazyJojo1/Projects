...
rownames(FreeThrows) <- c("KobeBryant","JoeJohnson","LeBronJames","CarmeloAnthony",
                          "DwightHoward","ChrisBosh","ChrisPaul","KevinDurant",
                          "DerrickRose","DwayneWade")
colnames(FreeThrows) <- c("2005","2006","2007","2008","2009","2010","2011","2012","2013","2014")
...
rownames(FreeThrowAttempts) <- c("KobeBryant","JoeJohnson","LeBronJames","CarmeloAnthony",
                          "DwightHoward","ChrisBosh","ChrisPaul","KevinDurant",
                          "DerrickRose","DwayneWade")
colnames(FreeThrowAttempts) <- c("2005","2006","2007","2008","2009","2010","2011","2012","2013","2014")
...
#FreeThrow.Accuracy
FreeThrow.accuracy <- round(FreeThrows / FreeThrowAttempts, 2)
FreeThrow.accuracy

#FreeThrowAttempts/Games
FreeThrowAttempts.Games <- round(FreeThrowAttempts /Games,2)
FreeThrowAttempts.Games

#Only 2 and 3 point preference
Point.NotFreeThrows <- Points - FreeThrows
Point.NotFreeThrows

#Point.NotFreeThrows/FieldGoal
Preference.Style <- round(Point.NotFreeThrows / FieldGoals,2)
Preference.Style
#Code for visualization
data <- FreeThrow.accuracy
Data <- data[rows,columns,drop=F]
matplot(t(Data), type="b" , pch=15:18 , col=c(1:4,6))
legend("bottomleft",inset=.01,legend=Players[rows],pch=15:18,col=c(1:4,6),horiz=F)

#Building a function
plot <- function(data,rows=1:10,columns=1:10){
  Data <- data[rows,columns,drop=F]
  matplot(t(Data), type="b" , pch=15:18 , col=c(1:4,6))
  legend("bottomleft",inset=.01,legend=Players[rows],pch=15:18,col=c(1:4,6),horiz=F)
  
}
#using function to do visual analysis
plot(FreeThrow.accuracy,1:5)
#accuracy of 0.8 means 80% 
plot(FreeThrow.accuracy,6:10)
#Preference.style
plot(Preference.Style)
#Points which were not free throw per game
plot(Point.NotFreeThrows/Games)
plot(Points/Games)
